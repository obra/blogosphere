...
**Part B (sheets), 2026-09-23.** Tasks B1–B4 as planned; sheet styles live in a new `app-macos-surfaces.css` (the chrome file hit Biome's 300-line limit), covered by the CSS scoping test. Verified live: Publish via Entry › Publish… and the toolbar, New Draft refused while it's open, focus landing on the first field, Escape (as a DOM event; real keystrokes were blocked by another app's secure input, then the screen locked) closing it. Code review (/par, 6 + 6 findings, all fixed): a conflict vanishing (pull merged it, sync detached) left `conflictSheetPath` set and the window silently modal, now cleared with every status; Quick Open, Settings and the Activity popover could stack with sheets (now one modal surface at a time via `modalOpen`, and a sheet replaces the popover); a selection change retargeted an open Publish sheet (now it dismisses it); Escape in the merge editor threw the hand merge away (now it means Back, and the merge text lives in the dialog); off macOS the auto-opened conflict dialog bypassed the one-sheet rule (now it goes through `openConflict` and waits for other sheets); sheets didn't take focus; other Escape handlers didn't claim the key; the Entry menu looked enabled under a sheet. Row conflict symbol → phase 4 (as planned).

**Part C (toast routing), 2026-09-23.** Tasks C1–C5 as planned, plus: `refresh`/search moved to `state.listActions.ts` and `addToast` to `state.noticeActions.ts` (file-length limits). Not verified live: the dev app shares the real GitHub token, so Save & Sync would push to the blog, and the screen was locked; the HUD, alerts, and deploy line rest on component and store tests, and are on Jesse's manual list. Code review (/par, 6 + 3 findings, all fixed): a failed deploy was cleared by any background pull (now only a new deploy or a Sync Now / Save & Sync clears it); the alert queue dropped a duplicate message's retry, which could lose a second entry's edits (now one alert per message runs every waiting retry, and falls back to a toast if the alert can't show); autosave failures would re-alert on every typing burst (now `source: "autosave"` shows "Couldn't save · Try Again" in that entry's save status until a save lands); a failed reload with entries showing vanished (now logged in Activity); the alert's second button is Cancel so Escape dismisses it.

**Part D (Settings window), 2026-09-23.** Tasks D1–D6 as planned, plus: the native-window hooks moved from AppShell to `windowHooks.ts` (file limits); `connect` lives in `src/bootstrap/connect.ts` with its errors described by `connectErrors.ts`. Found live: Tauri's window size includes the title bar the webview sits below, so the page adds that difference when fitting itself (the capability gained `allow-inner-size` and `allow-scale-factor`). Verified live: Settings… opens one "Blogosphere Settings" window, a second Settings… reuses it, it fits its content exactly, a template saved there reaches the main window (and was put back), a window command it wasn't granted is refused, and closing the main window with Settings open quits the app. Scenario cards: `docs/superpowers/e2e/phase3-surfaces.md`. Not verified: an iOS build (the iOS Rust target isn't installed here). Code review (/par, 3 + 4 findings, all fixed): two quick ⌘, could build two Settings windows (a lock now serializes open-or-focus); a failed template save looked saved in the Settings window (now stored before the state changes); connect errors matched on message text that the real client never produces (now by `GitHubError.kind`, tested with the real client's errors); a slow token check outlived the Settings window's wait (now 20s); replacing a token during the first download could run two downloads at once (now chained).
